    ...
    def add(self, b_matrix):
        if self.rows != b_matrix.rows or self.cols != b_matrix.cols:
            ...
            while (a_pos < len(self.compact_matrix) and b_pos < len(b_matrix.compact_matrix)):
                ...
                if self.compact_matrix[a_pos][0] == b_matrix.compact_matrix[b_pos][0] and self.compact_matrix[a_pos][1] == b_matrix.compact_matrix[b_pos][1]:
                        result.compact_matrix.append(b_matrix.compact_matrix[b_pos])
                        result.compact_matrix[-1][2] += self.compact_matrix[a_pos][2]
                        a_pos += 1
                        b_pos += 1
                # a item is before
                elif (self.compact_matrix[a_pos][0] < b_matrix.compact_matrix[b_pos][0]) or (self.compact_matrix[a_pos][0] == b_matrix.compact_matrix[b_pos][0] and self.compact_matrix[a_pos][1] < b_matrix.compact_matrix[b_pos][1]):
        ...
        if self.cols != b_matrix.rows:
            ...
            while a_pos<len(self.compact_matrix):
                ...
                while b_pos<len(b_trans.compact_matrix):
                    c = self.compact_matrix[b_pos][0] # current column in result matrix
                    
                    temp_a = a_pos
                    temp_b = b_pos
                    sum = 0
                    while (temp_a < len(self.compact_matrix) and self.compact_matrix[temp_a][0] == r and  temp_b < len(b_trans.compact_matrix) and b_trans.compact_matrix[temp_b][0] == c):
                    ...
                    while b_pos < len(b_trans.compact_matrix) and b_trans.compact_matrix[b_pos][0] == c: # jump to next column 
                        b_pos += 1
                        
                while a_pos < len(self.compact_matrix) and self.compact_matrix[a_pos][0] == r: # jump to next row 
                    a_pos +=1
            return result
